...
```{r}
MA_cities <- tibble(
  name = c("Boston", "Worcester"), 
  lat = c(42.3601, 42.2626), 
  lon = c(-71.0589, -71.8023)
)
...
ggplot() +
  geom_sf(data = MA_sf) +
  geom_sf(data = MA_cities_sf, aes(col =name), size = 3)
...
MA_cities <- tibble(
  name = c("Boston", "Worcester"), 
  lat = c(40.8448,40.6782),
  lon = c(-73.8648, -73.9442)
)
MA_cities_sf <- MA_cities %>% 
  st_as_sf(coords = c("lon", "lat"), crs = 4326)
...
NY_counties <- 
  USAboundaries::us_counties(resolution = "low", states = c("new york")) %>% 
  select(-state_name)
...
```
```{r}
 census_api_key("e18e3fcf282c184d73a9eb5076b0cd28590c48b9",overwrite=TRUE, install=TRUE)
```
